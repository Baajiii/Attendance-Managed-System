...
@Component({
  selector: 'app-editattendanceforstaff',
  templateUrl: './editattendanceforstaff.component.html',
  styleUrls: ['./editattendanceforstaff.component.css']
})
export class EditattendanceforstaffComponent {
...
list: NodeListOf<HTMLElement>;
    ...
    const storedRowData = sessionStorage.getItem('rowData');
     ...
     if (storedRowData) {
      try {
        ...
        if (this.rowData && this.rowData.name) { // Ensure data exists
          ...
          console.error('Invalid rowData structure:', this.rowData);
        }
      } catch (error) {
  ...
  this.EditModelobj.studentname = this.editAttendanceForm.value.name;
